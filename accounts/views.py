...
def activate(request, uidb64, token):
    ...
    except request(TypeError, ValueError, OverflowError, User.DoesNotExist):
        user = None
...
def login(request):
    if request.user.is_authenticated:
        ...
        if User.objects.filter(email=email).exists():
            checkUserActive = User.objects.get(email__exact =email)
            ...
            if user is not None:
                ...
                return redirect('myAccount')
            elif checkUserActive.is_active == False:
                messages.error(request, 'User not active yet')
                return redirect('login')        
            else:
                messages.error(request, 'Invalid login credentials')
                return redirect('login')
        else:
            messages.error(request, 'User doesn\'t exist in our system')
            return redirect('login')
...
def forgot_password(request):
    if request.method == 'POST':
        ...
        if User.objects.filter(email=email).exists():
            ...
            messages.error(request, 'The provided email address doesn\'t exist in our system')
            return redirect('login')
